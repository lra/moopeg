...
void Msg(TCHAR *szFormat, ...)
{
    ...
    MessageBox(NULL, szBuffer, TEXT("Moopeg Message"), MB_OK);
}
...
void EnableSizeMenu(BOOL bEnable)
{
    WPARAM nItems[5] = {ID_FILE_SIZE_HALF,    ID_FILE_SIZE_DOUBLE,
                        ID_FILE_SIZE_NORMAL,  ID_FILE_SIZE_THREEQUARTER,
                        ID_FILE_FULLSCREEN};
    ...
    switch(message)
    {
        ...
        case WM_KEYDOWN:
            ...
            switch(toupper((int) wParam))
            {
                // Frame stepping
                case '1':
                    PostMessage(ghApp, WM_COMMAND, ID_FILE_SIZE_HALF, 0);
					break;
                case '2':
                    PostMessage(ghApp, WM_COMMAND, ID_FILE_SIZE_NORMAL, 0);
					break;
                case '3':
                    PostMessage(ghApp, WM_COMMAND, ID_FILE_SIZE_DOUBLE, 0);
					break;

                case VK_LEFT:       // Reduce playback speed by 25%
                    PostMessage(ghApp, WM_COMMAND, ID_RATE_DECREASE, 0);
                    break;

                case VK_RIGHT:      // Increase playback speed by 25%
                    PostMessage(ghApp, WM_COMMAND, ID_RATE_INCREASE, 0);
                    break;

                case VK_DOWN:       // Set playback speed to normal
                    PostMessage(ghApp, WM_COMMAND, ID_RATE_NORMAL, 0);
                    break;

                case VK_SPACE:
                case 'P':
                    PostMessage(ghApp, WM_COMMAND, ID_FILE_PAUSE, 0);
                    break;

                case 'S':
                    PostMessage(ghApp, WM_COMMAND, ID_FILE_STOP, 0);
                    break;

                case 'M':
                    PostMessage(ghApp, WM_COMMAND, ID_FILE_MUTE, 0);
                    break;

                case 'F':
                case VK_RETURN:
					PostMessage(ghApp, WM_COMMAND, ID_FILE_FULLSCREEN, 0);
                    break;
                ...
                case VK_ESCAPE:
                    if (g_bFullscreen)
                        ...
                        PostMessage(ghApp, WM_COMMAND, ID_FILE_CLOSE, 0);
                    break;
				...
				case 'O':
					PostMessage(ghApp, WM_COMMAND, ID_FILE_OPEN, 0);
					break;
            }
            break;
...
int PASCAL WinMain(HINSTANCE hInstC, HINSTANCE hInstP, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    wc.hIcon         = LoadIcon(hInstC, MAKEINTRESOURCE(IDI_MOOPEG));
    if(!RegisterClass(&wc))
    {
